...
BEGIN
 ...
 //In het volgende deel worden part1 en part2 ingevuld of niet naargelang de ontvangende cellen hoger/even hoog liggen of tot een ander perceel behoren
  ...
  IF ((PART1=0.0)AND(PART2=0.0)) THEN // no cells were found (both have a lower elevation or another parcel ID)
       BEGIN
...
begin
   ...
   if Buffermap[i,j] <= Number_of_Buffers then      // center of the buffer drains to lowest neighbour
   begin
     w := 1;
     Minimum := 99999999.9;
     check := false;
...
Calculate_routing(Routing); //The flow direction(s) is calulated for every gridcell + the relative contribution to every receiving neighbour
