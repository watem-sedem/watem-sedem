...
uses
  Classes, SysUtils, FileUtil, Dialogs,RData_CN, GData_CN, ReadInParameters, Raster_calculations, math,
  ...
  for k:=-1 to 1 do
   for l:=-1 to 1 do
    begin
      ...
      IF DTM[i,j]>extremum then extremum:=DTM[i+k,j+l];
    end;
  If extremum < (DTM[i,j]+A[i,j]) then
   begin
    A[i,j]:=extremum-DTM[i,j];
...
procedure Calculatewaterero(i,j:integer);   //waterero in meter
...
if (Include_dam) AND (Dam_map[i,j] <> 0) then        // adjust C factor, P factor and kTc for dams
begin
 C_factor[i,j]:=0.01;
 P_factor[i,j]:=1;
 ktc[i,j]:=7;
 ...
 if (Include_ditch) AND (Ditch_map[i,j] <> 0) then       // adjust C factor, P factor and kTc for ditches
begin
 C_factor[i,j]:=0;
 P_factor[i,j]:=1;
 ktc[i,j]:=9999;
...
// in oude versie W/S model werd een buffer rondom een rivier gecreëerd met een zeer hoge KTc => hier ook nodig?
 ...
 // Erosion equations
 RUSLE[i,j]:=RFactor*C_factor[i,j]*P_factor[i,j]*K_Factor[i,j]*LS[i,j];       // in kg/m²
 ...
 Ero_Potential:=RUSLE[i,j]*area/BD;                      // in m³
