...
This overview is derived from {ref}``inireference``. For each input in
the ``ini`` file, it defines: - the name (key) of the variable - the
data type of the variable - the default value (if available) - the model
part for which this is relevant - Is it (for the given model part)
M(andatory) of C(onditionally mandatory)?

The format need to be improved but it provides a single/complete general
...
L model -> Desmet1996\_McCool, Desmet1996\_Vanoost2003 S model ->
Desmet1996, Nearing1997
